...
# Convert a DenseAxisArray to an AxisArray.
AxisArrays.AxisArray(daa::JuMP.Containers.DenseAxisArray) = AxisArray(daa.data, daa.axes)

# Convert a SparseAxisArray to a Dict. 
getdict(saa::JuMP.Containers.SparseAxisArray) = saa.data
...
function readresults(model::ModelInfo, status::MOI.TerminationStatusCode)
    ...
    cost = AxisArray(JuMP.value.(Systemcost))
    emis = AxisArray(JuMP.value.(CO2emissions))
    fuel = AxisArray(JuMP.value.(FuelUse))
    # getting Electricity for all set combos is slowest, so let's optimize storage format and use faster internal function call 
    elec = Dict((k,c) => [JuMP.value.(Electricity[r,k,c,h]) for h in HOUR, r in REGION] for k in TECH for c in CLASS[k]);
    charge = AxisArray(JuMP.value.(Charging))
    # oops, StorageLevel is also slow
    storage = Dict((k,c) => [JuMP.value.(StorageLevel[r,k,c,h]) for h in HOUR, r in REGION] for k in storagetechs for c in STORAGECLASS[k]);
...
function analyzeresults(results)
    ...
    function chart(country::Symbol; plotstoragetech=:none, plotbatterycharge=false, plotbatterylevel=false, optionlist...)
        ...
        composite = plot(; layout=grid(2,3,widths=[.2,.1,.2,.2,.1,.2]), size=(1850,950),    
